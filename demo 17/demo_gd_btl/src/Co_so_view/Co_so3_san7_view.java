...
public class Co_so3_san7_view extends JPanel {
	...
	public Co_so3_san7_view() {
		...
		JLabel lblNewLabel_3 = new JLabel(" DS đặt sân bóng 7 người Cơ sở 3");
		lblNewLabel_3.setFont(new Font("Tahoma", Font.PLAIN, 20));
		lblNewLabel_3.setBounds(10, 0, 427, 34);
		...
		lb_ten_san.setBounds(312, 130, 79, 27);
		panel.add(lb_ten_san);


		list_sb = new ArrayList<>();
		list_sb= san_bong_dao.getInterface().select_by_condition("iD_CoSo = 3 and LoaiSan = '7 người'");
		...
		jcombo_ten_san.setBounds(401, 126, 146, 30);
		...
		txt_gio_nghi.setBounds(109, 196, 146, 27);
					...
					if (mt.find()) {
						...
						if (gio_bat_dau.before(now)) {
							...
							if (check_thoi_gian_da(gio_bat_dau, gio_ket_thuc) == false) {
								...
								if (dat_san_dao.getInterface().check_san(date_format, h_Da, h_Nghi, "3",
										jcombo_ten_san.getItemAt(jcombo_ten_san.getSelectedIndex())) == false) {
									JOptionPane.showMessageDialog(null, "Cơ sở 3 đã hết sân khung giờ " + h_Da + "-"
									...
									ds.setMa_san(dat_san_dao.getInterface().lay_ma_san(date_format, h_Da, h_Nghi, "3",jcombo_ten_san.getItemAt(jcombo_ten_san.getSelectedIndex())));
									ds.setMaCS("3");
		...
		bt_dat_san.setBounds(369, 260, 97, 46);
		...
		bt_huy_san.setBounds(476, 260, 97, 46);
		...
		bt_tim_san.setBounds(253, 260, 106, 46);
		...
		btn_thanhToan.setBounds(583, 260, 153, 46);
		panel.add(btn_thanhToan);
		
	}

	public void showTable() {
		list = dat_san_dao.getInterface().select_by_condition("iD_CoSo = 3 and LoaiSan = '7 người'");
		int i = 1;
		DefaultTableModel model = (DefaultTableModel) table.getModel();
		for (dat_san ds : list) {
