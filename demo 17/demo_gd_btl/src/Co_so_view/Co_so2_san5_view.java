...
public class Co_so2_san5_view extends JPanel {
	...
	public Co_so2_san5_view() {
		...
		JLabel lblNewLabel_3 = new JLabel(" DS đặt sân bóng 5 người Cơ sở 2 ");
		lblNewLabel_3.setFont(new Font("Tahoma", Font.PLAIN, 20));
		lblNewLabel_3.setBounds(10, 0, 436, 34);
		...
		lb_ten_san.setBounds(323, 124, 79, 27);
		panel.add(lb_ten_san);


		list_sb = new ArrayList<>();
		list_sb= san_bong_dao.getInterface().select_by_condition("iD_CoSo = 2 and LoaiSan = '5 người'");
		...
		jcombo_ten_san.setBounds(398, 120, 146, 30);
		...
		txt_gio_nghi.setBounds(110, 198, 146, 27);
					...
					if (mt.find()) {
						...
						if (gio_bat_dau.before(now)) {
							...
							if (check_thoi_gian_da(gio_bat_dau, gio_ket_thuc) == false) {
								...
								if (dat_san_dao.getInterface().check_san(date_format, h_Da, h_Nghi, "2",
										jcombo_ten_san.getItemAt(jcombo_ten_san.getSelectedIndex())) == false) {
									JOptionPane.showMessageDialog(null, "Cơ sở 2 đã hết sân khung giờ " + h_Da + "-"
									...
									ds.setMa_san(dat_san_dao.getInterface().lay_ma_san(date_format, h_Da, h_Nghi, "2",jcombo_ten_san.getItemAt(jcombo_ten_san.getSelectedIndex())));
									ds.setMaCS("2");
	...
	public void showTable() {
		list = dat_san_dao.getInterface().select_by_condition("iD_CoSo = 2 and LoaiSan = '5 người'");
		int i = 1;
		DefaultTableModel model = (DefaultTableModel) table.getModel();
		for (dat_san ds : list) {
